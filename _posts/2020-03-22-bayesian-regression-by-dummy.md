...
resources that skipped over a (few) fairly tricky, important step(s). 
I thought it would be useful for me to talk through this process to ensure
I understand it fairly well, and it might be useful for somebody either as a 
sanity check for their work or a quick walkthrough.

In Bayesian linear regression, we posit that the vector of $$n$$ observations,
$$y$$ was generated by a linear process such that
$$y = X\beta + \epsilon$$.[^footnote]
 Here $$ X $$ is the $$n \times k$$ design matrix,
...
there. The referenced book[^ohagan] gives slightly more detail, mentioning that we 
...
With this rewritten likelihood, we can search for a conjugate prior. O'Hagan
describes this portion in a bit more detail - I may revisit it at some point!


[^footnote]: Note that I am not implying a causal relationship between $$ X $$ and $$ y $$.
[^ohagan]: O'Hagan, Anthony (1994). Bayesian Inference. Kendall's Advanced Theory of Statistics. 2B (First ed.). Halsted. ISBN 0-340-52922-9.
